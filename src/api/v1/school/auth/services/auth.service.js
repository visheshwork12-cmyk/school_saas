...
class AuthService {
  ...
  async login(credentials, tenant) {
    try {
      ...
      const user = await this.userRepo.findByEmailAndSchool(email, schoolId, true);
      ...
      const refreshToken = jwtService.sign(payload, config.jwt.refreshExpiresIn, config.jwt.refreshSecret);
