...
const getTrialLimits = () => {
  try {
    ...
    return baseConfig?.subscription?.plans?.TRIAL?.limits || 
...
class TenantService {
  static async validateTenant(tenantId, context = {}) {
    try {
      ...
      if (!mongoose.connection || mongoose.connection.readyState !== 1) {
        logger.warn(`Database not connected, using mock tenant for: ${tenantId}`);
        return {
          tenantId,
      ...
      if (!tenant) {
        ...
        if (!tenant) {
          throw new BusinessException(`Tenant not found or inactive: ${tenantId}`, HTTP_STATUS.CLIENT_ERROR.NOT_FOUND);
        }
      ...
      if (tenant.subscription?.endDate && new Date(tenant.subscription.endDate) < new Date()) {
        throw new BusinessException(`Tenant subscription expired: ${tenantId}`, HTTP_STATUS.CLIENT_ERROR.FORBIDDEN);
      }
      ...
      logger.debug(`Tenant validated: ${tenantId}`, { tenantName: tenant.name });
      return tenant;
    ...
    try {
      ...
      const tenantData = {
        ...
        subscription: {
          ...
          startDate: new Date(),
          endDate: new Date(Date.now() + defaultTrialDays * 24 * 60 * 60 * 1000),
          features: data.subscription?.features || ['ACADEMIC', 'ATTENDANCE'],
          limits: data.subscription?.limits || trialLimits,
        },
        createdBy: context.userId ? new mongoose.Types.ObjectId(context.userId) : null,
        updatedBy: context.userId ? new mongoose.Types.ObjectId(context.userId) : null,
      };
      ...
      try {
        await CacheService.invalidate(`tenant:${tenant.tenantId}`, tenant.tenantId);
      } catch (cacheError) {
      ...
      logger.error(`Tenant creation failed: ${error.message}`, { tenantId: data.tenantId });
      
      try {
        await AuditService.log('TENANT_CREATION_FAILED', { 
          tenantId: data.tenantId, 
          error: error.message 
        }, context);
      } catch (auditError) {
  ...
  static async updateSubscription(tenantId, subscription, context) {
    try {
      ...
      if (!tenant) {
        throw new BusinessException(`Tenant not found: ${tenantId}`, HTTP_STATUS.CLIENT_ERROR.NOT_FOUND);
      }
      ...
      try {
        await AuditService.log('SUBSCRIPTION_UPDATED', {
          tenantId,
      ...
      logger.info(`Subscription updated for tenant: ${tenantId}`, { plan: subscription.plan });
      return tenant.toObject();

    } catch (error) {
      logger.error(`Subscription update failed: ${error.message}`, { tenantId });
      
      try {
        await AuditService.log('SUBSCRIPTION_UPDATE_FAILED', { 
          tenantId, 
          error: error.message 
        }, context);
      } catch (auditError) {
  ...
  static async deactivateTenant(tenantId, context) {
    try {
      ...
      tenant.updatedBy = context.userId ? new mongoose.Types.ObjectId(context.userId) : null;
      ...
      try {
        await AuditService.log('TENANT_DEACTIVATED', {
          tenantId,
          name: tenant.name,
      ...
      logger.error(`Tenant deactivation failed: ${error.message}`, { tenantId });
      
      try {
        await AuditService.log('TENANT_DEACTIVATION_FAILED', { 
          tenantId, 
          error: error.message 
        }, context);
      } catch (auditError) {
