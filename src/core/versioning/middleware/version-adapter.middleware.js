...
const versionAdapterMiddleware = catchAsync(async (req, res, next) => {
  ...
  if (publicEndpoints.includes(req.path)) {
    ...
    logger.debug(`Public endpoint accessed, using default version: ${req.path}`, {
  ...
  if (!VersionHandlerService.isCompatible(clientVersion, baseConfig.versioning.currentApiVersion)) {
    ...
    throw new BusinessException(`Incompatible version: ${clientVersion}`, HTTP_STATUS.BAD_REQUEST);
  }
  ...
  if (versionStatus.status === 'DEPRECATED' || versionStatus.status === 'MAINTENANCE') {
    res.setHeader('X-Version-Warning', `${versionStatus.status} version, sunset: ${versionStatus.sunsetDate || 'TBD'}`);
  }
  ...
  logger.debug(`Version middleware applied: ${clientVersion}`, { requestId: req.requestId });
  next();
});
