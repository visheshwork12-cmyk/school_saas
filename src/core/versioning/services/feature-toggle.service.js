...
class FeatureToggleService {
    ...
    try {
      ...
      if (enabled === null) {
        ...
        await CacheService.set(
          cacheKey,
          enabled,
          baseConfig.featureFlags.cacheTtl,
          context.tenantId,
        );
      }

      if (!enabled) {
        await AuditService.log("FEATURE_DISABLED", {
          feature,
          tenantId: context.tenantId,
        });
        logger.debug(`Feature disabled: ${feature}`, {
          tenantId: context.tenantId,
        });
      }
  ...
  static applyContextChecks(feature, context) {
    ...
    if (
      ...
      !semver.satisfies(context.clientVersion, `>=${reqs.minVersion}`)
    ) {
      return false;
    }
