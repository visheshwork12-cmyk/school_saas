...
class JWTManager {
        ...
        const strategy = new JwtStrategy(options, async (req, payload, done) => {
            try {
                ...
                if (!payload.sub || !payload.organizationId || !payload.schoolId) {
                    throw new AuthenticationException('Invalid token payload structure', HTTP_STATUS.UNAUTHORIZED);
                }

                // Validate tenant
                const tenant = await TenantService.validateTenant(payload.organizationId, {
                ...
                if (!user) {
                    ...
                    return done(new AuthenticationException('User not found or inactive', HTTP_STATUS.UNAUTHORIZED), null);
                }
                ...
                logger.debug(`JWT authentication successful for user: ${user._id}`, { requestId: req.requestId });
                return done(null, userContext);
            } catch (error) {
                logger.error(`JWT authentication failed: ${error.message}`, { requestId: req.requestId });
    ...
    static generateAccessToken(payload, expiresIn) {
        try {
            ...
            throw new AuthenticationException('Token generation failed', HTTP_STATUS.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    static generateRefreshToken(payload, expiresIn) {
        try {
            ...
            throw new AuthenticationException('Refresh token generation failed', HTTP_STATUS.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    static verifyAccessToken(token) {
        try {
            return jwt.verify(token, this.#config.jwt.accessSecret, { algorithms: ['HS256'] });
        } catch (error) {
            logger.warn(`Access token verification failed: ${error.message}`);
            throw new AuthenticationException('Invalid or expired access token', HTTP_STATUS.UNAUTHORIZED);
        }
    }
    ...
    static verifyRefreshToken(token) {
        try {
            return jwt.verify(token, this.#config.jwt.refreshSecret, { algorithms: ['HS256'] });
        } catch (error) {
            logger.warn(`Refresh token verification failed: ${error.message}`);
            throw new AuthenticationException('Invalid or expired refresh token', HTTP_STATUS.UNAUTHORIZED);
        }
    }
