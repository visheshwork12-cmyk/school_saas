 ...
 */
const roleGuard = (requiredRoles) => catchAsync(async (req, res, next) => {
  try {
    // Ensure user is authenticated
    if (!req.user || !req.user._id) {
      throw new AuthenticationException('User not authenticated', { requestId: req.requestId });
    }

    // Ensure tenant context is set
    if (!req.context?.tenantId) {
      throw new AuthenticationException('Tenant context missing', { requestId: req.requestId });
    }
    ...
    if (!tenant.subscription?.plan || !isPlanAuthorized(tenant.subscription.plan, requiredRoles)) {
      throw new AuthenticationException(
    ...
    logger.debug(`Role access granted: ${req.user._id}`, { roles: userRoles, tenantId: req.context.tenantId });
    next();
  } catch (error) {
