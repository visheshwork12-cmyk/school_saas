...
class ApplicationConfig {
  ...
  loadConfiguration() {
    const baseConfig = {
      ...
      database: {
        mongodb: {
          uri:
            ...
            "mongodb://localhost:27017/school-erp-dev",
          options: {
      ...
      subscription: {
        ...
        gracePeriodDays:
          parseInt(process.env.SUBSCRIPTION_GRACE_PERIOD_DAYS) || 7,
        plans: {
          trial: {
            name: "Trial",
            duration: parseInt(process.env.DEFAULT_TRIAL_DAYS) || 30,
            features: (
              process.env.TRIAL_FEATURES || "ACADEMIC,ATTENDANCE"
            ).split(","),
            limits: {
          ...
          basic: {
            name: "Basic",
            features: (
              process.env.BASIC_FEATURES || "ACADEMIC,ATTENDANCE"
            ).split(","),
            limits: {
          ...
          premium: {
            ...
            features: (
              ...
              "ACADEMIC,ATTENDANCE,FINANCE,LIBRARY"
            ).split(","),
            limits: {
      ...
      email: {
        ...
        templates: {
          ...
          path:
            process.env.EMAIL_TEMPLATES_PATH || "./src/shared/templates/email",
        },
        queue: {
      ...
      versioning: {
        ...
        minimumSupportedVersion:
          process.env.MINIMUM_SUPPORTED_VERSION || "1.0.0",
  ...
  isContainerEnvironment() {
    return Boolean(
      process.env.KUBERNETES_SERVICE_HOST || process.env.DOCKER_CONTAINER,
    );
  }
  ...
  validateConfiguration() {
    ...
    if (missingVars.length > 0) {
      ...
      if (this.environment === "production") {
        ...
        logger.warn(
          "Using default values for missing environment variables in non-production environment",
        );
      }
    }
    ...
    if (this.environment === "production") {
      if (this.config.auth.jwt.accessSecret.length < 32) {
        throw new Error(
          "JWT_ACCESS_SECRET must be at least 32 characters long in production",
        );
      }
