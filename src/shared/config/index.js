...
const loadConfig = () => {
  ...
  try {
    ...
    const config = merge({}, baseConfig, envConfig, {
      paths: {
        docs: 'docs', // Added to fix undefined error
      },
    });
    ...
    const schema = Joi.object({
      ...
      env: Joi.string().valid('development', 'production', 'staging', 'test', 'local').required(),
      appName: Joi.string().default('School ERP SaaS'),
      mongo: Joi.object({
        uri: Joi.string().uri().required(),
        options: Joi.object({
          maxPoolSize: Joi.number().min(1).default(10),
          serverSelectionTimeoutMS: Joi.number().default(15000), // Fixed from previous issue
    ...
    if (error) {
      ...
      logger.error(`Configuration validation failed: ${errorMessage}`, { env });
      throw new Error(`Configuration validation failed: ${errorMessage}`);
    }

    // Ensure JWT compatibility
    if (value.jwt.secret && !value.jwt.accessSecret) {
      value.jwt.accessSecret = value.jwt.secret;
      value.jwt.refreshSecret = value.jwt.secret;
    }

    // Ensure auth configuration
    if (!value.auth) {
      value.auth = {
        maxLoginAttempts: 5,
