...
const productionConfig = {
  ...
  mongo: {
    ...baseConfig.mongo,
    uri: process.env.MONGODB_URI || throwError('MONGODB_URI required in production'),
    options: {
  ...
  redis: {
    ...baseConfig.redis,
    enabled: true,
    host: process.env.REDIS_HOST || throwError('REDIS_HOST required in production'),
    port: parseInt(process.env.REDIS_PORT) || 6379,
    password: process.env.REDIS_PASSWORD || throwError('REDIS_PASSWORD required in production'),
    // Production-optimized settings
  ...
  cors: {
    ...baseConfig.cors,
    allowedOrigins: process.env.CORS_ALLOWED_ORIGINS?.split(',') || throwError('CORS_ALLOWED_ORIGINS required in production'),
    credentials: true,
  ...
  jwt: {
    ...baseConfig.jwt,
    accessSecret: process.env.JWT_ACCESS_SECRET || throwError('JWT_ACCESS_SECRET required in production'),
    refreshSecret: process.env.JWT_REFRESH_SECRET || throwError('JWT_REFRESH_SECRET required in production'),
  ...
  security: {
    ...
    contentSecurityPolicy: {
      ...
      directives: {
        ...
        imgSrc: ["'self'", 'data:', process.env.AWS_S3_BUCKET ? `https://${process.env.AWS_S3_BUCKET}.s3.amazonaws.com` : null].filter(Boolean),
      },
    },
  ...
  logging: {
    ...
    transports: {
      ...
      cloudWatch: {
        enabled: baseConfig.deployment.platform === 'aws-lambda' && Boolean(process.env.CLOUDWATCH_LOG_GROUP),
  ...
  smtp: {
    ...baseConfig.smtp,
    host: process.env.SMTP_HOST || throwError('SMTP_HOST required in production'),
    port: parseInt(process.env.SMTP_PORT) || 587,
    secure: process.env.SMTP_SECURE === 'true',
    user: process.env.SMTP_USER || throwError('SMTP_USER required in production'),
    pass: process.env.SMTP_PASS || throwError('SMTP_PASS required in production'),
  ...
  aws: {
    ...baseConfig.aws,
    accessKeyId: process.env.AWS_ACCESS_KEY_ID || throwError('AWS_ACCESS_KEY_ID required in production'),
    secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY || throwError('AWS_SECRET_ACCESS_KEY required in production'),
    s3: {
      ...baseConfig.aws.s3,
      bucket: process.env.AWS_S3_BUCKET || throwError('AWS_S3_BUCKET required in production'),
      signedUrlExpiresIn: 3600,
      maxFileSize: '50mb',
    },
    ses: {
      ...baseConfig.aws.ses,
      fromEmail: process.env.SES_FROM_EMAIL || throwError('SES_FROM_EMAIL required in production'),
    },
    cloudWatch: {
      enabled: true,
      logGroupName: process.env.CLOUDWATCH_LOG_GROUP,
...
function throwError(variable) {
  throw new BusinessException(`${variable} must be set in production environment`);
}
