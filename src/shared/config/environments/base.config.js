...
const baseConfig = {
  ...
  mongo: {
    ...
    options: {
      ...
      maxPoolSize: deploymentInfo.isServerless
        ? 1
        : parseInt(process.env.MONGO_MAX_POOL_SIZE) || 10,
      minPoolSize: deploymentInfo.isServerless
        ? 0
        : parseInt(process.env.MONGO_MIN_POOL_SIZE) || 2,
      serverSelectionTimeoutMS: deploymentInfo.isServerless
        ? 5000
        : parseInt(process.env.MONGO_SERVER_SELECTION_TIMEOUT) || 10000,
      socketTimeoutMS: deploymentInfo.isServerless
        ? 30000
  ...
  cache: {
    ...
    maxKeys: deploymentInfo.isServerless
      ? 1000
      : parseInt(process.env.CACHE_MAX_KEYS) || 10000,
    // Cache strategy
  ...
  security: {
    ...
    contentSecurityPolicy: {
      ...
      directives: {
        ...
        connectSrc: [
          "'self'",
          process.env.REDIS_HOST ? `https://${process.env.REDIS_HOST}` : "*",
        ],
  ...
  rateLimit: {
    ...
    max: deploymentInfo.isServerless
      ? 100
  ...
  bodyParser: {
    jsonLimit: deploymentInfo.isServerless
      ? "10mb"
      : process.env.BODY_PARSER_JSON_LIMIT || "1mb",
    urlencodedLimit: deploymentInfo.isServerless
      ? "10mb"
  ...
  compression: {
    ...
    level: deploymentInfo.isServerless
      ? 1
      : parseInt(process.env.COMPRESSION_LEVEL) || 6,
  ...
  server: {
    timeout: deploymentInfo.isServerless
      ? 30000
      : parseInt(process.env.SERVER_TIMEOUT) || 30000,
    keepAliveTimeout: deploymentInfo.isServerless
      ? 5000
      : parseInt(process.env.KEEP_ALIVE_TIMEOUT) || 65000,
    headersTimeout: deploymentInfo.isServerless
      ? 6000
  ...
  logging: {
    ...
    maxFiles: deploymentInfo.isServerless
      ? 1
      : parseInt(process.env.LOG_MAX_FILES) || 5,
    maxSize: deploymentInfo.isServerless
      ? "10m"
    ...
    transports: {
      ...
      cloudWatch: {
        enabled:
          deploymentInfo.platform === "aws-lambda" &&
  ...
  subscription: {
    ...
    plans: {
      ...
      BASIC: {
        features: (
          process.env.BASIC_FEATURES || "ACADEMIC,ATTENDANCE,COMMUNICATION"
        ).split(","),
        limits: {
      ...
      PREMIUM: {
        features: (
          process.env.PREMIUM_FEATURES ||
          "ACADEMIC,ATTENDANCE,FINANCE,LIBRARY,HR,TRANSPORT"
        ).split(","),
        limits: {
  ...
  auth: {
    ...
    enableSuspiciousActivityDetection:
      process.env.ENABLE_SUSPICIOUS_ACTIVITY_DETECTION === "true",
  ...
  versioning: {
    ...
    deprecationWarningDays:
      parseInt(process.env.VERSION_DEPRECATION_WARNING_DAYS) || 90,
    sunsetGracePeriodDays:
      parseInt(process.env.VERSION_SUNSET_GRACE_PERIOD_DAYS) || 180,
    // Version strategy
  ...
  featureFlags: {
    ...
    defaultRolloutPercentage:
      parseInt(process.env.FEATURE_ROLLOUT_DEFAULT_PERCENTAGE) || 10,
    enableExperimentalFeatures:
      process.env.ENABLE_EXPERIMENTAL_FEATURES === "true",
    abTestDurationDays:
      parseInt(process.env.A_B_TEST_DEFAULT_DURATION_DAYS) || 30,
  ...
  aws: {
    ...
    s3: {
      ...
      signedUrlExpiresIn:
        parseInt(process.env.S3_SIGNED_URL_EXPIRES_IN) || 3600,
      maxFileSize: process.env.S3_MAX_FILE_SIZE || "10mb",
      allowedFileTypes: (
        process.env.S3_ALLOWED_FILE_TYPES || "image/*,application/pdf,text/*"
      ).split(","),
    },
    ses: {
  ...
  optimizations: {
    ...
    serverless: {
      ...
      minimalLogging:
        deploymentInfo.isServerless && process.env.NODE_ENV === "production",
    },
