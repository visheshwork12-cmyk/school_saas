...
class RedisConfig {
  ...
  async createClient(purpose = "default") {
    ...
    try {
      ...
      logger.error(
        `❌ Failed to create Redis client for ${purpose}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async connectWithRetry(client, purpose) {
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        logger.warn(
          `Redis connection attempt ${attempt}/${this.maxRetries} failed for ${purpose}: ${error.message}`,
        );
    ...
    for (const [purpose, client] of this.clients) {
      shutdownPromises.push(
        client
          ...
          .catch((error) => {
            logger.warn(
              `⚠️ Error disconnecting Redis client ${purpose}: ${error.message}`,
            );
          }),
      );
    }
...
export const createRedisClients = (purposes) =>
  redisConfig.createClients(purposes);
