...
const runSeed = async () => {
  try {
    ...
    const plans = [
      { id: 'BASIC', features: ['ACADEMIC', 'ATTENDANCE'], limits: { students: 100, teachers: 10, storage: 1 } },
      // Add more
    ];

    // Create default organization
    const defaultOrg = await OrganizationModel.create({ name: 'Default Org', tenantId: 'default' });

    // Create subscriptions
    for (const plan of plans) {
      await SubscriptionModel.create({ organizationId: defaultOrg._id, planId: plan.id, features: plan.features, limits: plan.limits });
    }
