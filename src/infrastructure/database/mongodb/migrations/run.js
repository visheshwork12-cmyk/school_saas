// src/infrastructure/database/mongodb/migrations/run.js - Enhanced migration system
...
class MigrationManager {
  ...
  async getCompletedMigrations(connection) {
    const Migration = connection.model('Migration', new mongoose.Schema({
  ...
  determineMigrationsToRun(migrations, completed, direction, target) {
    if (direction === 'up') {
      ...
      const completedMigrations = migrations.filter(m => completed.includes(m.id));
      completedMigrations.reverse();
      
      if (target) {
        const targetIndex = completedMigrations.findIndex(m => m.id === target);
        return targetIndex >= 0 ? completedMigrations.slice(0, targetIndex + 1) : [];
      }
  ...
  async runSingleMigration(connection, migration, direction) {
    ...
    try {
      logger.info(`${direction === 'up' ? '⬆️' : '⬇️'} Running migration: ${migration.name}`);
      ...
      // Record migration
      await this.recordMigration(connection, migration, direction, executionTime);
      
      logger.info(`✅ Migration completed: ${migration.name} (${executionTime}ms)`);
      
    } catch (error) {
      const executionTime = Date.now() - startTime;
      logger.error(`❌ Migration failed: ${migration.name} (${executionTime}ms)`, {
        error: error.message,
        stack: error.stack
      });
      throw error;
  ...
  async acquireLock(connection, migrationId) {
    const Lock = connection.model('MigrationLock', new mongoose.Schema({
  ...
  async status() {
    ...
    try {
      ...
      for (const migration of migrations) {
        const status = completed.includes(migration.id) ? '✅ Applied' : '⏳ Pending';
        console.log(`${status} ${migration.name} - ${migration.description}`);
      }
      
      console.log('='.repeat(50));
      console.log(`Total: ${migrations.length}, Applied: ${completed.length}, Pending: ${migrations.length - completed.length}`);
  ...
  try {
    switch (command) {
      ...
      case 'status':
        ...
        break;
      default:
        console.log('Usage: node run.js [up|down|status] [target]');
