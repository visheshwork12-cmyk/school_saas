...
class DatabaseIndexManager {
  ...
  async createStudentIndexes(connection) {
    ...
    await this.createIndex(Student, 'students', [
      ...
      { fields: { organizationId: 1, studentId: 1 }, options: { unique: true } },
      { fields: { organizationId: 1, rollNumber: 1, academicYear: 1 }, options: { unique: true } },
      ...
      // Student information
      { fields: { organizationId: 1, admissionNumber: 1 }, options: { unique: true } },
  ...
  async createClassIndexes(connection) {
    ...
    await this.createIndex(Class, 'classes', [
      // Primary indexes
      { fields: { organizationId: 1, name: 1, academicYear: 1 }, options: { unique: true } },
  ...
  async createAttendanceIndexes(connection) {
    ...
    await this.createIndex(Attendance, 'attendance', [
      // Primary tracking
      { fields: { organizationId: 1, studentId: 1, date: 1 }, options: { unique: true } },
  ...
  async createGradeIndexes(connection) {
    ...
    await this.createIndex(Grade, 'grades', [
      // Primary indexes
      { fields: { organizationId: 1, studentId: 1, subjectId: 1, examId: 1 }, options: { unique: true } },
  ...
  async createFeeIndexes(connection) {
    ...
    await this.createIndex(Fee, 'fees', [
      // Primary tracking
      { fields: { organizationId: 1, studentId: 1, feeType: 1, academicYear: 1, term: 1 }, options: { unique: true } },
  ...
  async createLibraryIndexes(connection) {
    ...
    await this.createIndex(Book, 'books', [
      ...
      { fields: { organizationId: 1, title: 'text', author: 'text', publisher: 'text' } },
  ...
  async createTransportIndexes(connection) {
    ...
    await this.createIndex(Transport, 'transport', [
      { fields: { organizationId: 1, studentId: 1, routeId: 1 }, options: { unique: true } },
  ...
  async createCompoundIndexes(connection) {
    ...
    const User = connection.model('User');
    await this.createSingleIndex(User, 
    ...
    const Student = connection.model('Student');
    await this.createSingleIndex(Student,
    ...
    const Attendance = connection.model('Attendance');
    await this.createSingleIndex(Attendance,
  ...
  async createTextSearchIndexes(connection) {
    ...
    const Teacher = connection.model('Teacher');
    await this.createSingleIndex(Teacher, {
      firstName: 'text',
      lastName: 'text',
      employeeId: 'text',
      email: 'text',
  ...
  async createGeospatialIndexes(connection) {
    ...
    const School = connection.model('School');
    await this.createSingleIndex(School, { location: '2dsphere' }, { name: 'school_location' });

    // Transport route indexes
    const Transport = connection.model('Transport');
    await this.createSingleIndex(Transport, { 'route.coordinates': '2dsphere' }, { name: 'transport_route' });
    await this.createSingleIndex(Transport, { 'pickup.coordinates': '2dsphere' }, { name: 'transport_pickup' });
    await this.createSingleIndex(Transport, { 'drop.coordinates': '2dsphere' }, { name: 'transport_drop' });
  }
  ...
  async createIndex(Model, collectionName, indexes) {
    ...
    for (const indexConfig of indexes) {
      await this.createSingleIndex(Model, indexConfig.fields, indexConfig.options);
    }
  }
  ...
  async createSingleIndex(Model, indexFields, options = {}) {
    try {
      ...
      if (error.code === 11000 || error.codeName === 'IndexOptionsConflict') {
        logger.debug(`Index already exists with different options: ${error.message}`);
        this.indexStats.skipped++;
      } else {
...
export const dropAllIndexes = (tenantId, excludeBuiltIn) => indexManager.dropAllIndexes(tenantId, excludeBuiltIn);
